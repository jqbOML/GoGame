...
public class Interpreter extends AbstractInterpreter {
     ...
     */
    private ArrayList<Kamien> czyOddech(Kamien kamien, ArrayList<Kamien> sprawdzoneKamienie){
        ...
        if(czySasiedniePoleWolne(kamien))
        {
            ...
            sprawdzoneKamienie.add(kamien);
            System.out.println("Dodano kamien[" + kamien.wezX() + "][" + kamien.wezY() + "]");

            if (x < 18 && !uduszoneKamienie.contains(planszaGo[x + 1][y]) && planszaGo[x + 1][y] != null && planszaGo[x + 1][y].wezKolor() == kamien.wezKolor()) {
                czyOddech(planszaGo[x + 1][y], sprawdzoneKamienie);
            }

            if (x > 0 && !uduszoneKamienie.contains(planszaGo[x - 1][y]) && planszaGo[x - 1][y] != null && planszaGo[x - 1][y].wezKolor() == kamien.wezKolor()) {
                czyOddech(planszaGo[x - 1][y], sprawdzoneKamienie);
            }

            if (y < 18 && !uduszoneKamienie.contains(planszaGo[x][y + 1]) && planszaGo[x][y + 1] != null && planszaGo[x][y + 1].wezKolor() == kamien.wezKolor()) {
                czyOddech(planszaGo[x][y + 1], sprawdzoneKamienie);
            }

            if (y > 0 && !uduszoneKamienie.contains(planszaGo[x][y - 1]) && planszaGo[x][y - 1] != null && planszaGo[x][y - 1].wezKolor() == kamien.wezKolor()) {
                czyOddech(planszaGo[x][y - 1], sprawdzoneKamienie);
            }
        }
